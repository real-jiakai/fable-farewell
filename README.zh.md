...
2026 年 7 月 7 日之后，Fable 5 将从 Claude 订阅计划（Pro / Max / Team / 部分 Enterprise）中下线，转为按用量计费。它**并没有被淘汰或弃用**——在此期间，它始终可以通过 Claude API 完整使用，不受这次调整影响。Anthropic 表示这只是暂时调整，会在算力允许的情况下尽快让 Fable 5 重新成为订阅计划的标准模型。倒计时的目标时刻是 `2026-07-08T06:59:59Z`（即 2026 年 7 月 7 日太平洋时间当天结束）——这是一个估算值，因为官方从未公布具体的切换时间。
